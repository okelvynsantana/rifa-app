...
import { CheckCircle, Copy, Upload, Tag } from 'lucide-react'
...
export default function ModalCompra({ rifa, numerosSelecionados, onClose, onSuccess }: Props) {
  ...
  const promoAtiva = temPromocao && numerosSelecionados.length >= (rifa.min_numeros_promocao ?? 2)
  const precoUnitario = promoAtiva ? rifa.preco_promocional! : rifa.preco_numero
  const total = numerosSelecionados.length * precoUnitario
  const totalSemPromo = numerosSelecionados.length * rifa.preco_numero
  const economia = totalSemPromo - total
  ...
  function handleClose() {
    if (step === 'sucesso') { onSuccess(); router.refresh() }
    else onClose()
  }
    ...
    <Modal isOpen title={stepTitles[step]} onClose={handleClose} size="md">
      ...
      {step !== 'sucesso' && (
        <div className="px-5 py-2 flex items-center gap-2">
          {(['dados', 'pix', 'comprovante'] as Step[]).map((s, i) => {
            const stepOrder = ['dados', 'pix', 'comprovante']
        ...
        <div className="mx-5 mb-3">
          <div className={`rounded-xl p-3 ${promoAtiva ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50'}`}>
            <div className="flex items-center justify-between mb-2">
              ...
              {promoAtiva && (
                <span className="flex items-center gap-1 text-xs bg-emerald-500 text-white font-bold px-2 py-0.5 rounded-full">
                  <Tag size={10} />
                  -{desconto}% promo
                </span>
              )}
            ...
            <div className="flex items-end gap-2">
              ...
              {promoAtiva && economia > 0 && (
                <p className="text-xs text-gray-400 line-through mb-0.5">{formatCurrency(totalSemPromo)}</p>
              )}
            </div>
            {promoAtiva && economia > 0 && (
              <p className="text-xs text-emerald-600 font-semibold mt-0.5">
                Você economizou {formatCurrency(economia)}! 🎉
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-5 pb-5 space-y-3">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (v.length > 7) fmt = `(${v.slice(0, 2)}) ${v.slice(2, 7)}-${v.slice(7, 11)}`
      ...
      {step === 'pix' && (
        <div className="px-5 pb-5 space-y-4">
          <div className="bg-gray-50 rounded-2xl p-4 text-center">
            <p className="text-xs text-gray-500 mb-1">Faça o PIX de</p>
            <p className="font-black text-2xl text-gray-900">{formatCurrency(total)}</p>
            <p className="text-xs text-gray-500 mt-1">para {rifa.nome_pix}</p>
          </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
            <p className="text-amber-700 text-xs">
              ⚠️ Após pagar, clique em "Já paguei" para enviar o comprovante. Seus números ficam reservados por 2 horas.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" onClick={() => setStep('dados')}>Voltar</Button>
            <Button onClick={() => setStep('comprovante')}>Já paguei ✓</Button>
          </div>
        </div>
      )}
      ...
      {step === 'comprovante' && (
        <div className="px-5 pb-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" onClick={() => setStep('pix')}>Voltar</Button>
            <Button disabled={!comprovante} loading={loading} onClick={handleEnviarComprovante}>Enviar</Button>
          </div>
        </div>
      )}
          ...
          <div className="bg-gray-50 rounded-xl p-3 mb-6">
            ...
            {promoAtiva && economia > 0 && (
              <p className="text-emerald-600 text-xs font-bold mt-2">
                Você economizou {formatCurrency(economia)} com a promoção! 🎉
              </p>
            )}
          </div>
          <Button className="w-full" size="lg" onClick={handleClose}>Fechar</Button>
        </div>
      )}
    </Modal>
  )
