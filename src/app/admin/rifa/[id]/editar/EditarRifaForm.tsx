...
export default function EditarRifaForm({ rifa: initialRifa }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const [imagemFile, setImagemFile] = useState<File | null>(null)
  const [imagemPreview, setImagemPreview] = useState<string | null>(initialRifa.imagem_url)
  const [imagemPremioFile, setImagemPremioFile] = useState<File | null>(null)
  const [imagemPremioPreview, setImagemPremioPreview] = useState<string | null>(initialRifa.imagem_premio_url)

  const [promocaoAtiva, setPromocaoAtiva] = useState(
    !!(initialRifa.preco_promocional && initialRifa.min_numeros_promocao)
  )
  const [form, setForm] = useState({
    titulo: initialRifa.titulo,
    descricao: initialRifa.descricao || '',
    regras: initialRifa.regras || '',
    preco_numero: String(initialRifa.preco_numero),
    preco_promocional: String(initialRifa.preco_promocional || ''),
    min_numeros_promocao: String(initialRifa.min_numeros_promocao || '2'),
    data_sorteio: initialRifa.data_sorteio.split('T')[0],
    premio: initialRifa.premio,
    chave_pix: initialRifa.chave_pix,
    nome_pix: initialRifa.nome_pix,
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      let imagem_url = imagemPreview === null ? null : initialRifa.imagem_url
      let imagem_premio_url = imagemPremioPreview === null ? null : initialRifa.imagem_premio_url

      if (imagemFile) {
        const uploaded = await uploadImagem(imagemFile, 'rifa')
        if (uploaded) imagem_url = uploaded
      }
      if (imagemPremioFile) {
        const uploaded = await uploadImagem(imagemPremioFile, 'premio')
        if (uploaded) imagem_premio_url = uploaded
      }
      ...
      }).eq('id', initialRifa.id)

      if (error) throw error

      toast.success('Rifa atualizada!')
      router.push(`/admin/rifa/${initialRifa.id}`)
      router.refresh()
    } catch (err) {
      console.error(err)
  ...
  const desconto = precoNormal > 0 && precoPromo > 0 ? Math.round((1 - precoPromo / precoNormal) * 100) : 0

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/admin/rifa/${initialRifa.id}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Preço */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
          <h2 className="font-bold text-gray-900 text-sm">Preço e Data</h2>
            ...
            <div className="space-y-3">
              ...
              {desconto > 0 && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-emerald-700">{desconto}% de desconto</p>
                    <p className="text-xs text-emerald-600">
                      Comprando {minPromo}+ números: R$ {precoPromo.toFixed(2)} cada
                    </p>
                  </div>
                </div>
              )}
        ...
        {preview ? (
          <>
            ...
            <button
              ...
              className="absolute top-2 right-2 w-7 h-7 bg-red-600 text-white rounded-full flex items-center justify-center"
            >
              <X size={14} />
            </button>
          </>
